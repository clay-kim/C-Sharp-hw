...
{
    // Two classes that can be used to create a simple text menuing system
    //     <T> is the parameter that is passed to any menuitem's Action
    ...
    public class TextMenu<T>
    {
        ...
        //// 4) removing menu item?
        public void RemoveItem(TextMenuItem<T> item)
        {
            if (item == null)
                throw new ArgumentNullException("Cannot remove null menuitem to menu");
